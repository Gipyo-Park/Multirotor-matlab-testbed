...
% (LQI) for the quadrotor. A lower level controller takes those inputs and 
% controls the error between the deisred and actual Euler angles. 

function attitude_LQI
...
% phi error
phi_error_I = phi - Quad.phi_des;
phi_error = phi - Quad.phi_des;
% phi_error = phi;
% phi_error = -Quad.phi_des;
% phi_error = 0;
if(abs(phi_error_I) < Quad.phi_KI_lim)
    phi_error_sum = phi_error_sum + phi_error_I;
end
% phi_dot error
...
% theta error
theta_error_I = theta - Quad.theta_des;
theta_error = theta - Quad.theta_des;
% theta_error = theta;
% theta_error = -Quad.theta_des;
% theta_error = 0;
if(abs(theta_error_I) < Quad.theta_KI_lim)
    theta_error_sum = theta_error_sum + theta_error_I;
end
% theta_dot error
...
% psi error
psi_error_I = psi - Quad.psi_des;
psi_error = psi - Quad.psi_des;
% psi_error = psi;
% psi_error = -Quad.psi_des;
% psi_error = 0;
if(abs(psi_error_I) < Quad.psi_KI_lim)
    psi_error_sum = psi_error_sum + psi_error_I;
end
% psi_dot error
...
x_state = [phi_error; phi_dot_error; theta_error; theta_dot_error; psi_error; psi_dot_error; phi_error_sum; theta_error_sum; psi_error_sum];

% LQI 이득 적용 (3x9 * 9x1 = 3x1)
tau_LQI = -Quad.K_LQI_attitude * x_state;
