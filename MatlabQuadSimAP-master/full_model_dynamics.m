...
X_dot = [phi_dot; phi_ddot; theta_dot; theta_ddot; psi_dot; psi_ddot; x_dot; x_ddot; y_dot; y_ddot; z_dot; z_ddot];
LQR_df_dx = jacobian(X_dot,[phi; phi_dot; theta; theta_dot; psi; psi_dot; x; x_dot; y; y_dot; z; z_dot]); % phi_dot ≈ p , theta_dot ≈ q , psi_dot ≈ r 
LQR_df_du = jacobian(X_dot,[ft; tau_x; tau_y; tau_z]);

% 평형점에서 평가 (phi=0, phi_dot=0, theta=0, theta_dot=0, psi=0, psi_dot=0)
A_eq = double(subs(LQR_df_dx, {phi,phi_dot,theta,theta_dot,psi,psi_dot,ft}, {0, 0, 0, 0, 0, 0, m*g}));
B_eq = double(subs(LQR_df_du, {phi,phi_dot,theta,theta_dot,psi,psi_dot,ft}, {0, 0, 0, 0, 0, 0, m*g}));
...
B_LQI = [B_eq;
         zeros(3,3)];
Q_LQI = diag([1, 1, 1, 1, 1, 1, 10, 10, 10]);  % 상태 + 적분 오차
R_LQI = diag([1, 1, 1]);

% LQI 이득 계산
K_LQI_attitude = lqr(A_LQI, B_LQI, Q_LQI, R_LQI);
